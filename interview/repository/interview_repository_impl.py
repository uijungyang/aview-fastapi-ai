...
class InterviewRepositoryImpl(InterviewRepository):
    ...
    def generateFirstFollowup(
        ...
        prompt = (
            ...
            f"아래 면접자의 답변과 기본 정보를 참고해서, 짧고 명확한 꼬리 질문을 하나 생성해줘\n\n"
            ...
            f"- 질문만 출력하고, 줄바꿈(\n)으로 구분해줘\n"
            f"- 질문은 반드시 **한 문장**으로 구성할 것\n"
            f"- 설명이나 추가 문장은 절대 포함하지 말 것\n"
            f"- 중복된 내용 없이, 질문은 짧고 명확하게 작성할 것\n"
            f"- 질문만 출력하고, 다른 문장은 포함하지 말 것\n"
            f"- 질문이 여러 가지를 묻는 복합 질문이 되지 않도록 주의해.\n"
        )
    ...
    def generateProjectFollowupQuestion(
        ...
        if projectExperience == "프로젝트 경험 있음":
            prompt = (
                f"너는 기술 면접관이야. 다음 면접자의 이전 답변과 질문 ID를 참고해서, 그에 대한 심화 질문 또는 꼬리 질문을 하나 생성해줘.\n\n"
                f"- 이전 질문 ID(questionId)와 면접자의 답변(answerText)를 기반으로 관련성 높은 후속 질문을 생성해야 해.\n\n"
                f"요청사항:\n"
                f"- 면접자는 총 1개의 질문을 받게 됩니다.\n"
                f"- 질문 하나 → 답변 → 다음 질문 순으로 진행됩니다.\n"
                f"- 질문은 짧고 명확하게, 설명 없이 한 문장으로 출력하고,다른 문장은 포함하지 말 것"
                f"- 질문은 반드시 **한 문장**으로 구성할 것\n"
                f"- 설명이나 추가 문장은 절대 포함하지 말 것\n"
                f"- 질문이 여러 가지를 묻는 복합 질문이 되지 않도록 주의해.\n"
            )
