...
if not os.path.exists(SPK_MODEL_PATH):
    print("Please download the speaker model from "
          f"https://alphacephei.com/vosk/models and unpack as {SPK_MODEL_PATH} "
...
# rec = KalidRecognizer(model, wf.getframerate(), spk_model)
rec = KaldiRecognizer(model, wf.getframerate())
rec.SetSpkModel(spk_model)


# Compare speakers with cosine distance.
# Can keep one or several fingerprints for the speaker in a database
...
print(
    "Note that second distance is not very reliable because utterance is too short. "
